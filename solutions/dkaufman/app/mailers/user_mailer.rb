class UserMailer < ActionMailer::Base
  ...
  def invite_admin_email(store_id, admin_hex, email)
    ...
    mail(:to => email,
         :subject => "You have been invited to be an admin of #{store_name}.")
  end
  ...
  def invite_stocker_email(store_id, admin_hex, email)
    ...
    store_name = Store.find(store_id).name
    mail(:to => email,
         :subject => "You have been invited to be a stocker of #{store_name}.")
  end

  def alert_stocker_email(store_id, email)
    @store = Store.find(store_id)
    mail(:to => email,
         :subject => "You have been added as a stocker of #{@store.name}.")
  end

  def fire_admin(store_id, email)
    store_name = Store.find(store_id).name
    mail(:to => email,
         :subject => "You have been fired from your position at #{store_name}.")
  end
  ...
  def alert_admin_email(store_id, email)
    @store = Store.find(store_id)
    mail(:to => email,
         :subject => "You have been added as an admin of #{@store.name}.")
  end
end
